...
ssize_t r_write(int fd, void *buf, size_t size) {
	char * bufp			= NULL;
	size_t bytestowrite = 0, byteswritten = 0, totalbytes = 0;
...
ssize_t r_read(int fd, void *buf, size_t size) {
	ssize_t retval = 0;
	while (retval = read(fd, buf, size), retval == -1 && errno == EINTR) continue;
	return retval;
}

int r_close(int fildes) {
	int retval = 0;
	while (retval = close(fildes), retval == -1 && errno == EINTR) continue;
	return retval;
}

int r_open2(const char *path, int oflag) {
	int retval = 0;
	while (retval = open(path, oflag), retval == -1 && errno == EINTR) continue;
	return retval;
}

int r_mkfifo(const char *path, int oflag) {
	int retval = 0;
	while (retval = mkfifo(path, oflag), retval == -1 && (errno == EINTR)) continue;
	return retval;
}