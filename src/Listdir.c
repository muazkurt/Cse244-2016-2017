...
#define SEM_NAME_LOG   "lzx"
#define SEM_NAME_COUNT "czx"
...
main_return_val *listDir_r(const parameters *request) {
    ...
    snprintf(path, PATH_MAX, "%s%s", request->base_path, request->referance_path);
    snprintf(fifoFilename, PATH_MAX, "%s.fifo", path);
    fprintf(stderr, "> (pid: %d) listDir path: %s%s\n", getpid(), request->base_path,
            request->referance_path);
                ...
                switch (dirContent->d_type) {
                    case DT_DIR:// Directory
                        ...
                        if (FAILURE_CASE == (childProcess = fork())) {
                            ...
                            snprintf(request->referance_path, PATH_MAX, "%s/%s",
                                     path + strlen(request->base_path), dirContent->d_name);
                            main_return_val *subdir_info = listDir_r(request);
                            if (NULL == subdir_info) {
                    ...
                    case DT_REG:// File
                    {
                        ...
                        p->base_path = request->base_path;
                        p->counter_sem = request->counter_sem;
                        p->log_sem = request->log_sem;
                        p->logFile = request->logFile;
                        p->query = request->query;
                        snprintf(p->referance_path, PATH_MAX, "%s/%s", request->referance_path,
                                 dirContent->d_name);
                        pthread_create(&thread_family[dir_info->total_files_main++], NULL,
                                       (void *) searchFile, p);
                    } break;
                    default:
    ...
    for (size_t i = 0; i < dir_info->total_files_main; ++i) {
        ...
        dir_info->total_occurences += file_info->result.occurences;
        dir_info->total_line_count += file_info->result.line_count;
        free(file_info);
    }
    ...
    if (NULL != openedDir && FAILURE_CASE == (fifoCreated = closedir(openedDir)))
        fprintf(stderr, "Failed while closing the directory %s", request->base_path);
...
int init(parameters *param) {
    ...
    if (0 != (retVal = pthread_rwlock_init(&param->logging, NULL))) {
        ...
        pthread_rwlock_destroy(&param->logging);
               ...
               (param->logFile = r_open3(LOG_FILENAME, O_CREAT | O_WRONLY | O_TRUNC, LOG_PERMS))) {
        fprintf(stderr, "Open %s is failed (pid: %d)", LOG_FILENAME, getpid());
        retVal = -1;
...
int listDir(const char *query, const char *path) {
    ...
    if (0 != init(&p))
        fprintf(stderr, "Failed to init (%d)", getpid());
    else {
        main_return_val *result = listDir_r(&p);
        char found_times[FOUNDABLE_MAX];
        int len = snprintf(found_times, FOUNDABLE_MAX, "%d %s were found in total.\n",
                           result->total_occurences, query);
        write(p.logFile, found_times, len * sizeof(char));
        destroy(&p);
        fprintf(stderr,
                ...
                result->total_occurences, result->subdir_members_main, result->total_files_main,
                result->total_line_count, result->total_files_main, getMaxWorkers());
    }
    gettimeofday(&tpend, NULL);
    long timedif = MILLION * (tpend.tv_sec - tpstart.tv_sec) + tpend.tv_usec - tpstart.tv_usec;
    fprintf(stderr, "Total run time, in milisecounds:\t%ld\n", timedif);
}